...
func TestExpr_Activation(t *testing.T) {
	...
	for _, test := range tests {
		sched, err := Standard.Parse(test.spec)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestExpr_NextWithTz(t *testing.T) {
	...
	for _, c := range runs {
		sched, err := Standard.Parse(c.spec)
		if err != nil {
			t.Error(err)
			continue
		}
